...
function MatchCard({m,isAdmin,onClick}){
  const isDone=m.winner!==null;const isGF=m.bracket==="GF";const isLFin=m.label==="Lower Final";
  return(<div onClick={()=>isAdmin&&onClick(m)} className={`rounded-lg border overflow-hidden w-44 transition-all ${isAdmin?"cursor-pointer hover:border-orange-500/60":""} ${isGF?"border-yellow-500/50 bg-yellow-500/5":isLFin?"border-purple-500/40 bg-purple-500/5":isDone?"border-zinc-700 bg-zinc-800/60":"border-zinc-700 bg-zinc-800/30"}`}>
    ...
    <div className={`flex items-center gap-2 px-2 py-1.5 border-b border-zinc-700/50 ${m.winner===m.team1?"bg-orange-500/10":""}`}><span className={`flex-1 text-xs font-bold truncate ${m.team1?m.winner===m.team1?"text-orange-300":"text-zinc-200":"text-zinc-600"}`}>{m.team1||"TBD"}</span>{isDone&&<span className={`font-mono text-xs font-black w-5 text-center rounded ${m.winner===m.team1?"bg-orange-500 text-black":"text-zinc-500"}`}>{m.score1??""}</span>}</div>
    <div className={`flex items-center gap-2 px-2 py-1.5 ${m.winner===m.team2?"bg-orange-500/10":""}`}><span className={`flex-1 text-xs font-bold truncate ${m.team2?m.winner===m.team2?"text-orange-300":"text-zinc-200":"text-zinc-600"}`}>{m.team2||"TBD"}</span>{isDone&&<span className={`font-mono text-xs font-black w-5 text-center rounded ${m.winner===m.team2?"bg-orange-500 text-black":"text-zinc-500"}`}>{m.score2??""}</span>}</div>
  </div>);
}
function BracketColumn({label,rounds,isAdmin,onMatch}){
  return(<div className="flex flex-col gap-1 min-w-[11rem]"><div className="text-center text-zinc-500 font-mono text-[10px] uppercase tracking-wider mb-2 pb-1 border-b border-zinc-800">{label}</div><div className="flex flex-col gap-2">{rounds.map(m=><MatchCard key={m.id} m={m} isAdmin={isAdmin} onClick={onMatch}/>)}</div></div>);
}
function TournamentTab({isAdmin,setShowLogin}){
  ...
  if(view==="create")return(<div className="flex flex-col gap-5"><div className="flex items-center gap-3"><button onClick={()=>setView("list")} className="text-zinc-500 hover:text-zinc-300 text-sm font-mono">← Voltar</button><h2 className="text-orange-400 font-mono font-bold text-sm uppercase">Criar Torneio</h2></div><div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 flex flex-col gap-4"><div className="grid grid-cols-2 gap-4"><div className="flex flex-col gap-1"><label className="text-xs text-zinc-500 font-mono uppercase">Nome do Torneio</label><input value={tName} onChange={e=>setTName(e.target.value)} placeholder="Arena Cup" className="bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-sm text-zinc-200 focus:outline-none focus:border-orange-500"/></div><div className="flex flex-col gap-1"><label className="text-xs text-zinc-500 font-mono uppercase">Formato</label><div className="flex gap-2">{[{v:"md3",l:"MD3"},{v:"md1",l:"MD1"}].map(f=>(<button key={f.v} onClick={()=>setTFormat(f.v)} className={`flex-1 py-2 rounded-lg border font-mono text-sm font-bold transition-all ${tFormat===f.v?"border-orange-500 bg-orange-500/10 text-orange-400":"border-zinc-700 text-zinc-500 hover:border-zinc-600"}`}>{f.l}</button>))}</div></div></div><div className="flex flex-col gap-1"><label className="text-xs text-zinc-500 font-mono uppercase">Número de Times</label><div className="flex gap-2">{[4,8].map(n=>(<button key={n} onClick={()=>{setTCount(n);setTTeams(Array(n).fill(""));}} className={`px-6 py-2 rounded-lg border font-mono text-sm font-bold transition-all ${tCount===n?"border-orange-500 bg-orange-500/10 text-orange-400":"border-zinc-700 text-zinc-500"}`}>{n} times</button>))}</div></div><div><label className="text-xs text-zinc-500 font-mono uppercase block mb-2">Nomes dos Times (por seed)</label><div className="grid grid-cols-2 gap-2">{Array(tCount).fill(0).map((_,i)=>(<div key={i} className="flex items-center gap-2"><span className="text-zinc-600 font-mono text-xs w-6 text-right">{i+1}.</span><input value={tTeams[i]||""} onChange={e=>{const t=[...tTeams];t[i]=e.target.value;setTTeams(t);}} placeholder={`Time ${i+1}`} className="flex-1 bg-zinc-800 border border-zinc-700 rounded px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-orange-500"/></div>))}</div></div><button onClick={createTournament} disabled={!tName.trim()||creating} className="w-full bg-orange-500 hover:bg-orange-400 disabled:bg-zinc-700 text-black font-mono font-bold py-3 rounded-xl text-sm uppercase flex items-center justify-center gap-2">{creating&&<Spinner/>}Gerar Chaveamento ▶</button></div></div>);
  if(view==="bracket"&&current&&bv)return(<div className="flex flex-col gap-4"><div className="flex items-center justify-between"><div className="flex items-center gap-3"><button onClick={()=>setView("list")} className="text-zinc-500 hover:text-zinc-300 text-sm font-mono">← Voltar</button><div><h2 className="text-zinc-100 font-black text-lg">{current.name}</h2><span className="text-zinc-500 font-mono text-xs">{current.format.toUpperCase()} · {current.teamCount} times · Double Elimination</span></div></div>{isAdmin&&<span className="text-orange-400 font-mono text-xs border border-orange-500/30 px-2 py-1 rounded">Clique numa partida para editar</span>}{!isAdmin&&<button onClick={()=>setShowLogin(true)} className="text-zinc-500 font-mono text-xs border border-zinc-700 px-2 py-1 rounded hover:border-zinc-500">🔒 Editar</button>}</div><div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4"><div className="text-orange-400 font-mono text-xs font-bold uppercase mb-4">🏆 Winner Bracket</div><div className="flex gap-6 overflow-x-auto pb-2">{bv.wRounds.map((rnd,ri)=>(<BracketColumn key={ri} label={ri===bv.wRounds.length-1?"Winner Final":`Fase ${ri+1}`} rounds={rnd} isAdmin={isAdmin} onMatch={m=>{setEditMatch(m);setS1(m.score1??'');setS2(m.score2??'');}}/>))}{bv.gf&&<BracketColumn label="Grand Final" rounds={[bv.gf]} isAdmin={isAdmin} onMatch={m=>{setEditMatch(m);setS1(m.score1??'');setS2(m.score2??'');}}/>}</div></div><div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4"><div className="text-blue-400 font-mono text-xs font-bold uppercase mb-4">📉 Lower Bracket</div><div className="flex gap-6 overflow-x-auto pb-2">{bv.lRounds.map((rnd,ri)=>(<BracketColumn key={ri} label={`Perdedores F${ri+1}`} rounds={rnd} isAdmin={isAdmin} onMatch={m=>{setEditMatch(m);setS1(m.score1??'');setS2(m.score2??'');}}/>))}</div></div>{editMatch&&isAdmin&&(<div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center" onClick={()=>{setEditMatch(null);setS1("");setS2("");}}><div className="bg-zinc-900 border border-zinc-700 rounded-2xl p-6 w-80 shadow-2xl" onClick={e=>e.stopPropagation()}><h3 className="text-zinc-100 font-bold text-base mb-1">Resultado da Partida</h3><p className="text-zinc-500 text-xs mb-4 font-mono">{current.format.toUpperCase()} · {current.format==="md3"?"Melhor de 3 mapas (ex: 2-0, 2-1)":"Melhor de 1 mapa (ex: 13-7)"}</p><div className="flex flex-col gap-3 mb-4">{[{team:editMatch.team1,s:s1,setS:setS1},{team:editMatch.team2,s:s2,setS:setS2}].map(({team,s,setS},i)=>(<div key={i} className="flex items-center gap-3"><span className="text-zinc-200 font-bold text-sm flex-1">{team||"TBD"}</span><input type="number" value={s} onChange={e=>setS(e.target.value)} min="0" className="w-16 bg-zinc-800 border border-zinc-700 rounded px-2 py-1.5 text-center text-zinc-200 font-mono font-bold text-lg focus:outline-none focus:border-orange-500"/></div>))}</div><div className="flex gap-2"><button onClick={()=>{setEditMatch(null);setS1("");setS2("");}} className="flex-1 py-2 rounded-lg border border-zinc-700 text-zinc-400 font-mono text-sm">Cancelar</button><button onClick={saveMatchResult} disabled={s1===""||s2===""||Number(s1)===Number(s2)} className="flex-1 py-2 rounded-lg bg-orange-500 hover:bg-orange-400 disabled:bg-zinc-700 text-black font-mono font-bold text-sm">Salvar</button></div></div></div>)}</div>);
  return(<div className="flex flex-col gap-5"><div className="flex items-center justify-between"><div><h2 className="text-zinc-100 font-black text-xl">Tabelas</h2><p className="text-zinc-500 text-xs mt-0.5">Chaveamentos Double Elimination</p></div>{isAdmin?<button onClick={()=>{setView("create");setTName("");setTFormat("md3");setTCount(8);setTTeams(Array(8).fill(""));}} className="flex items-center gap-2 px-4 py-2 bg-orange-500 hover:bg-orange-400 text-black font-mono font-bold rounded-lg text-sm">＋ Novo Torneio</button>:<button onClick={()=>setShowLogin(true)} className="flex items-center gap-2 px-4 py-2 border border-zinc-700 text-zinc-400 font-mono rounded-lg text-sm hover:border-zinc-500">🔒 Login Admin</button>}</div>{loading&&<div className="flex justify-center py-12"><Spinner/></div>}<div className="flex flex-col gap-3">{tournaments.map(t=>(<div key={t.id} className="bg-zinc-900 border border-zinc-800 hover:border-zinc-700 rounded-xl p-4 flex items-center gap-4 transition-colors cursor-pointer" onClick={()=>{setCurrent(t);setView("bracket");}}><div className="w-10 h-10 rounded-lg bg-gradient-to-br from-orange-500 to-yellow-400 flex items-center justify-center font-black text-black text-sm shrink-0">🏆</div><div className="flex-1"><div className="text-zinc-100 font-bold">{t.name}</div><div className="text-zinc-500 text-xs mt-0.5 font-mono">{t.format.toUpperCase()} · {t.teamCount} times · Double Elimination · {new Date(t.createdAt).toLocaleDateString("pt-BR")}</div></div><div className="flex items-center gap-2"><span className="text-orange-400 font-mono text-xs border border-orange-500/30 px-2 py-1 rounded">Ver Chave →</span>{isAdmin&&<button onClick={e=>{e.stopPropagation();deleteTournament(t.id);}} className="text-zinc-600 hover:text-red-400 transition-colors text-sm px-1">✕</button>}</div></div>))}{tournaments.length===0&&!loading&&(<div className="text-center text-zinc-600 font-mono py-16 border border-dashed border-zinc-800 rounded-xl">Nenhum torneio criado ainda.<br/><span className="text-xs">{isAdmin?"Clique em '+ Novo Torneio' para começar.":"Faça login como admin para criar torneios."}</span></div>)}</div></div>);
}
  ...
  return(
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <div className="border-t border-zinc-900 mt-12 py-4 text-center">
        <span className="text-zinc-700 font-mono text-xs">RIVOTRICSMT — v4.0 — Firebase Realtime Database</span>
      </div>
    </div>
  );
}
